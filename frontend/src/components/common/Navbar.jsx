        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <span className="hidden sm:inline-block ml-2 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-medical-50 text-medical-700 border border-medical-200">
                Online Pharmacy
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
            ...
            {isAuthenticated && (
              <Link to="/profile" className="hover:text-medical-600 transition-colors">
                {currentT.profile}
              </Link>
            )}
            {isAdmin && (
              <Link
                to="/admin"
                className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-navy-900 text-white hover:bg-navy-800 transition-colors text-xs font-semibold"
              >
          ...
          <div className="flex items-center gap-3">
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                {/* Dropdown Menu */}
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50 animate-slide-up">
                    <div className="px-4 py-2.5 border-b border-gray-100">
                      <p className="text-xs text-gray-500">Kirilgan hisob:</p>
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="px-3.5 py-1.5 text-xs font-semibold text-navy-900 hover:text-medical-600 transition-colors"
                >
                  {currentT.login}
                </Link>
                <Link
                  to="/register"
                  className="px-3.5 py-1.5 text-xs font-semibold text-white bg-medical-600 hover:bg-medical-700 rounded-lg shadow-xs transition-colors"
                >
                  {currentT.register}
                </Link>
              </div>
          ...
          <div className="md:hidden py-3 border-t border-gray-100 flex flex-col gap-2">
            ...
            {isAdmin && (
              <Link
                to="/admin"
                onClick={() => setMobileMenuOpen(false)}
                className="px-3 py-2 rounded-lg text-sm font-semibold text-teal-700 bg-teal-50 flex items-center gap-2"
              >
